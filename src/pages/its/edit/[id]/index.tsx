...
import { useRoqClient, useItFindFirst } from 'lib/roq';
...
function ItEditPage() {
  ...
  const { data, error, isLoading, mutate } = useItFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ItInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.it.update({
        data: values as RoqTypes.it,
        where: {
          id,
        },
      });
